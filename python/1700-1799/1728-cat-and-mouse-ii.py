class Solution:
    def canMouseWin(self, grid: List[str], catJump: int, mouseJump: int) -> bool:
        ...
        def dp(cat: int, mouse: int, turn: int) -> bool:
            # We already searched the whole touchable grid
            if turn == nFloors * 2:
                return False
            ...
            if turn % 2 == 0:
                ...
                for k in range(4):
                    for jump in range(mouseJump + 1):
                        ...
                        if grid[x][y] == "#":
                            break
                        if grid[x][y] == "F":  # Mouse eats the food, so mouse wins
                            return True
                        if dp(cat, hash(x, y), turn + 1):
                            return True
                # Mouse can't win, so mouse loses
                return False
            else:
                # cat's turn
                i = cat // n
                ...
                for k in range(4):
                    for jump in range(catJump + 1):
                        ...
                        if grid[x][y] == "#":
                            break
                        if grid[x][y] == "F":  # Cat eats the food, so mouse loses
                            return False
                        nextCat = hash(x, y)
                        if nextCat == mouse:  # Cat catches mouse, so mouse loses
                            return False
                        if not dp(nextCat, mouse, turn + 1):
                            return False
                # Cat can't win, so mouse wins
                return True
